...
def read_annotations(img_dir):
    ...
    for ann_file_name in sorted(os.listdir(img_dir)): #creates a list of names in the given image directory (img_dir) #sorted() function 
        ext = os.path.splitext( ann_file_name )[1] #splits the pathname into the its name and its extension (basically ext is index 1 of the total file name eg:filenam.ext))

        if ext == '.txt':
            img = {'object':[]} #instantisation

            img_file_name = img_dir + ann_file_name # technically contains the absolute(abs) path to the file ann_file_name
           ...
           # print( "ann_file_name= ", ann_file_name)
            ...
            for line in file:  # each line is a keypoint pair to this picture # file is a list of strings read from the txt file where each line is treated as a string (return or enter at the end of each line is used to determine the end of each string)
                vWords = line.split() # splitting the line string into a list of elements, where each element is a sub string of the main string called 'line', using whitespace as the spacer element
                obj = {} #an empty dict is created

                img['filename'] = os.path.splitext( img_file_name )[0] + ".bmp"  # absolute path of the image file for the current annotation file being read, is stored as a string in img['filename'] # where img is a dictionary
                obj['name'] = vWords[0]  # in the empty obj dict a class name is created. #this name is appended by index 0 of vWords which is nothing but class
                #the following if block is for incrementing the number of classes in the empty list of seen_lables={}
                if obj['name'] in seen_labels:
                    seen_labels[obj['name']] += 1 #now as class (0) is now visible in seen_labels for every iteration 1 is appended
                else: 
                    seen_labels[obj['name']] = 1 #initally if obj['name'] is not in seen_labels then its appended by 1
                ...
                obj['x0'] = float( vWords[1] ) #x0
                obj['y0'] = float( vWords[2] ) #y0
                obj['x1'] = float( vWords[3] ) #x1
                obj['y1'] = float( vWords[4] ) #y1

                img['object'] += [obj]  # zu diesem img das keypoint-pair # The dict called obj is stored in the list called object, which is appended with a new dict instance of obj for every iteration of the loop
            ...
            if len(img['object']) > 0:  #if the object dict is no longer empty then store all the string of data stored in img, in list of all_imgs
                all_imgs += [img]
