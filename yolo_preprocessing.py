...
def read_annotations(img_dir, edge_threshold, side_threshold):
    ...
    for ann_file_name in sorted(
        os.listdir(img_dir)
    ):  # creates a list of names in the given image directory (img_dir) #sorted() function
        ext = os.path.splitext(ann_file_name)[
            1
        ...
        if ext == ".txt":
            ...
            for (
                ...
                file
            ):  # each line is a keypoint pair to this picture # file is a list of strings read from the txt file where each line is treated as a string (return or enter at the end of each line is used to determine the end of each string)
                vWords = (
                    line.split()
                ...
                if edgeVisibile(vWords[5]) and sideVisible(vWords[6]):

                    img["filename"] = (
                        os.path.splitext(img_file_name)[0] + ".bmp"
                    )  # absolute path of the image file for the current annotation file being read, is stored as a string in img['filename'] # where img is a dictionary
                    obj["name"] = vWords[
                        0
                    ]  # in the empty obj dict a class name is created. #this name is appended by index 0 of vWords which is nothing but class
                    # the following if block is for incrementing the number of classes in the empty list of seen_lables={}
                    if obj["name"] in seen_labels:
                        seen_labels[
                            obj["name"]
                        ] += 1  # now as class (0) is now visible in seen_labels for every iteration 1 is appended
                    else:
                        seen_labels[
                            obj["name"]
                        ] = 1  # initally if obj['name'] is not in seen_labels then its appended by 1
                    ...
                    img["object"] += [
                        obj
                    ]  # zu diesem img das keypoint-pair # The dict called obj is stored in the list called object, which is appended with a new dict instance of obj for every iteration of the loop
                    ...
                    print("Image was not considered for training \n")
...
class YoloBatchGenerator(Sequence):
            ...
            [
                ...
                iaa.SomeOf(
                    ...
                    [
                        ...
                        iaa.Multiply(
                            (0.5, 1.5), per_channel=0
                        ),  # change brightness of images (50-150% of original value)
                        iaa.ContrastNormalization(
                            (0.5, 2.0), per_channel=0
    ...
    def __getitem__(self, idx):  # liefert einen kompletten batch(Get a complete batch)
        ...
        if self.jitter:
            ia.seed(134)
            aug_pipe_det = (
                self.aug_pipe.to_deterministic()
            ...
            keypoints_batch_aug = aug_pipe_det.augment_keypoints(
                keypoints_on_images
            )  # augmented keypoints
        else:
            ...
            for obj in all_objs:  # each loop is for one part/shaft in the image
                if obj["name"] in self.config["LABELS"]:
                    ...
                    if (
                        ...
                        obj_indx = self.config["LABELS"].index(
                            obj["name"]
                        )  # label-class-number
                        ...
                        y_batch[
                            instance_count, grid_y, grid_x, best_anchor, 0
                        ] = kp0_x  # keypoint0 in grid-Koordinaten LUC
                        y_batch[instance_count, grid_y, grid_x, best_anchor, 1] = kp0_y
                        y_batch[instance_count, grid_y, grid_x, best_anchor, 2] = alpha
                        y_batch[
                            instance_count, grid_y, grid_x, best_anchor, 3
                        ] = 1.0  # confidence
                        y_batch[
                        ...
                        true_kpp_index += 1
                        true_kpp_index = (
                            true_kpp_index % self.config["TRUE_KPP_BUFFER"]
                        )  # avoid overflow
    ...
    def on_epoch_end(self):
        if self.shuffle:
            np.random.shuffle(self.images)  # shuffle along the first axis only


def edgeVisibile(__edge_visibility):
    if __edge_visibility < self.edge_threshold:
        return False
    else:
        return True
...
def sideVisible(__side_visibility):
    ...
    if __side_visibility < self.edge_threshold:
        return False
    else:
        return True
