...
def _main_(args):
    ...
    image = image[:, :, 1]
    # green channel only #index 1 represesnts the second position in 3rd dimention of the shape(128,128,3)where 3:= in BGR and 2nd position is G(green)

    image = np.expand_dims(image, -1)
    # lengths of dimensions [128,128,1] # This is because we lose one dimmension (colour depth) when we specify that values from the green channel only are to be used
    ...
    image = draw_kpp(
        image, grid_width, grid_height, kpp, config["model"]["labels"]
    )  # 4,4 sind die Ausgangsdimensionen des Grids
...
if __name__ == "__main__":
    ...
    args.input = "D:\\Master Project\\github\\kk\\Masters-Project\\Img_000205.bmp"
