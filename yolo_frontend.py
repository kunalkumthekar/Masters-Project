...
from keras.callbacks import EarlyStopping, ModelCheckpoint, TensorBoard #to make sure our monitoring parameter gets most improved results
...
class SpecialYOLO(object):
    def __init__(self, input_width,
                       input_height,
                       labels,
                       max_kpp_per_image ): #max_kpp_per_image is not required acc to professor
        ...
        self.anchors = [0.5,0.5]  # the model classifies and regresses bounding boxes with reference to anchor boxes of multiple scales and aspect ratios
        ...
        input_image    = Input(shape=(self.input_height, self.input_width, 1))# declaring the input image shape by input function, required for building conv layer, 1:=B&W
        ...
        x = Conv2D(16, (3,3), strides=(1,1), padding='same', name='conv_' + str( num_layer ), use_bias=False)(input_image)  #16 kernels of 3x3 size
        ...
        x = Conv2D(3+1+ self.nb_class, (3,3), strides=(1,1), padding='same', name='conv_'+str( num_layer ), use_bias=False)(x) #3:= x,y coord, angle between 2 points of kp
        x = BatchNormalization(name='norm_' + str( num_layer ))(x)                                                             #1:=1confidence of object in grid cell
        x = LeakyReLU(alpha=0.1)(x)
        num_layer += 1

        # make the object detection layer
        output = Conv2D(self.nb_kpp * (3 + 1 + self.nb_class),#(x,y,alpha+conf+no.of classes) #nb_classes are one hot encoded
                        (1,1), strides=(1,1),  #in this case (1,1) kernel is used which tightens up the possibility of probaibility extraction)
                        padding='same',
                        name='DetectionLayer',
        ...
        output = Reshape((self.grid_h, self.grid_w, self.nb_kpp, 3 + 1 + self.nb_class))(x) #this output detects the kpp with respect to the grid
    ...
    def custom_loss(self, y_true, y_pred):
        ...
        mask_shape = tf.shape(y_true)[:4] #for mask_shape [:4] represents:= (batch_size(:0), nb_grid_x(:1), nb_grid_y(:2), nb_anchors(:3)) 

        # cell_x and cell_y then contain their respective x and y coordinates for each grid_cell as lookup coordinates
        cell_x = tf.to_float( tf.reshape(tf.tile(tf.range(self.grid_w), [self.grid_h]), (1, self.grid_h, self.grid_w, 1, 1)))  #v1 .to.float depracated use tf.cast for v2
        ...
        cell_grid = tf.tile(tf.concat([cell_x,cell_y], -1), [self.batch_size, 1, 1, self.nb_kpp, 1]) 
        ...
        pred_kp0_xy = tf.sigmoid(y_pred[..., :2]) + cell_grid  # transform to grid coordinates, predicted keypoint0 grid coordinates are the first two elements in last dimension (x0,y0)

        pred_alpha = (y_pred[..., 2:3])  # predicted keypoint1 grid coordinates are the elements 2 to 3 in last dimension thats angle between (x1,y1)
        ...
        true_kpp_class_argmax = tf.argmax(y_true[..., 4:], -1) #axis=-1 represents the last axis (last element of the array)
        ...
        coord_mask = tf.expand_dims(y_true[..., 3], axis=-1) * self.coord_scale  #tf.expand_dims simply changes the dimension shape at the end(axis=-1)-> (x0,y0,x1,y1,confidence,nb_anchors)
        ...
        conf_mask = conf_mask + 1.0  #all set to 1 which were previously just an array of zeros(tf.zeros) set in the masking sections
        ...
        conf_mask = conf_mask + y_true[..., 3] * self.object_scale  # set the cells containing keypoints to 6 [prev conf.mask was set to 1...now confmask is added to nb_anchor and multiplied by 5(i.e object_sclae), this sets it to 6]
        ...
        true_kp0_xy,  coord_mask = tf.cond(tf.less(seen, self.warmup_batches+1),
        ...
        loss_alpha    = tf.reduce_sum(tf.square(true_alpha-pred_alpha) * coord_mask * self.direction_scale) / (nb_coord_kpp + 1e-6) / 2. #direction.scale from .json
        loss_conf  = tf.reduce_sum(tf.square(true_kpp_conf-pred_kpp_conf) * conf_mask)  / (nb_conf_kpp  + 1e-6) / 2. #1e-6 is just a small number which is used so that if nb_conf_kpp is 0, the fraction should not get invalid output
        ...
        loss = tf.cond(tf.less(seen, self.warmup_batches+1),
                      lambda: loss_kp0_xy + loss_alpha + loss_conf + loss_class + 10, # Adding bias as 10     #bias of 10 is added so that the activation function(leaky relu which limits till 10)is always activated
        ...
        if self.debug: #debug puts out an boolean output true/false
            ...
            total_recall = tf.assign_add(total_recall, current_recall) #it simply updates ref=total_recall and adds value=current_recall to it
    ...
    def load_weights(self, weight_path):  #this function is used to call the weights path which later called upon by yolo_predict file
    ...
    def normalize(self, image):  #CONVERTING THE IMAGE FORMAT FROM 0-255 to 0-1.
        return image / 255.  #returning the normalised image
    ...
    def train(self, train_imgs,     # the list of images to train the model
        ...
        optimizer = Adam(lr=learning_rate, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0) #parameters beta1 and beta2 cntrol the decay of the learning rate
        ...
        early_stop = EarlyStopping(monitor='val_loss',   #used to monitor validation_loss and helps in stopping the training val_loss doesnt reduce for more than 2/3 epochs
                           min_delta=0.001,
                           patience=3,#2or3
                           mode='min',
                           verbose=1)
        checkpoint = ModelCheckpoint(saved_weights_name,  #this callback saves the best model having the lowest monitoring factor(i.e val_loss)
                                     monitor='val_loss',
                                     verbose=1,
                                     save_best_only=True,
        ...
        image = cv2.imread("G:\\Shaft_Program_Files\\valid\\Img_000256.bmp") #just a sample test image
        image = image[:,:,0] # blue channel only # [dim_1 == img_width (length 128), dim_2 == img_height(length 128), dim_3 == colour channel(length 3)]  [128, 128, 3]
    ...
    def predict(self, image):
        ...
        image = self.normalize(image) # dividing the entire color depth(0-255) by 255. to get it into form (0-1)
